...
@section('title', 'Categorias de Na Mídia - Admin')
...
<div class="space-modern">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Categorias de Na Mídia</h1>
            <p class="text-gray-600 mt-2">Gerencie as categorias de mídia</p>
        </div>
    </div>
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg border">
                            <div class="flex items-center space-x-3">
                                <div class="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center">
                                    <i class="fas fa-photo-video text-primary-600"></i>
                                </div>
                                <div>
                                    <h3 class="font-medium text-gray-900">{{ $category->name }}</h3>
                                    <p class="text-sm text-gray-500">{{ $category->media_count ?? 0 }} itens</p>
                                </div>
                            </div>
                ...
                <form action="{{ route('admin.media-categories.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               placeholder="Ex: LED, Outdoor, Back Bus..."
                               required>
                        @error('name')
...
function deleteCategory(categoryId) {
    if (confirm('Tem certeza que deseja excluir esta categoria? Esta ação não pode ser desfeita.')) {
        fetch(`/admin/media-categories/${categoryId}`, {
            method: 'DELETE',
            headers: {
