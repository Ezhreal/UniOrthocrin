            ...
            <div class="space-modern-sm">
                ...
                <div>
                    ...
                    @if($media->thumbnail_path)
                        <p class="text-sm text-gray-600 mt-1">Atual: <a href="/{{ $media->thumbnail_path }}" target="_blank" class="text-primary-600 underline">ver thumbnail</a></p>
                    @endif
                    @error('thumbnail')
                ...
                <div class="grid-modern grid-modern-2">
                    ...
                    <div>
                        <label for="media_category_id" class="form-label-modern">Categoria *</label>
                        <select id="media_category_id" name="media_category_id" required
                                class="form-select-modern @error('media_category_id') border-error-500 @enderror">
                            <option value="">Selecione uma categoria</option>
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('media_category_id', $media->media_category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" 
                              ...
                              placeholder="Descreva o item...">{{ old('description', $media->description) }}</textarea>
                    @error('description')
                        <p class="form-error-modern">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="active" {{ old('status', $media->status) == 'active' ? 'selected' : '' }}>Ativo</option>
                        <option value="inactive" {{ old('status', $media->status) == 'inactive' ? 'selected' : '' }}>Inativo</option>
                    </select>
                    @error('status')
                        <p class="form-error-modern">{{ $message }}</p>
                    @enderror
                ...
                <div class="modern-card hover-modern-lift">
                    <div class="modern-card-header">
                        ...
                        <div>
                            ...
                            <div class="mt-4">
                                <h5 class="text-modern-body font-medium mb-3">Arquivos Existentes ({{ $media->files->count() }})</h5>
                    ...
                    <div class="space-modern-sm">
                        ...
                        <div class="pt-4 border-t border-gray-200">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-modern-body">{{ $media->created_at->format('d/m/Y') }}</span>
                            </div>
                            <div class="flex justify-between items-center mt-2">
                                <span class="text-modern-caption">Atualizado em:</span>
                                <span class="text-modern-body">{{ $media->updated_at->format('d/m/Y') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
