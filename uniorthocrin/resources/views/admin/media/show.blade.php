...
@section('title', 'Visualizar Item - Na Mídia')
...
<div class="space-modern">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-modern-title">{{ $media->name }}</h1>
            <p class="text-modern-subtitle">Detalhes do item de mídia</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('admin.media.edit', $media) }}" class="btn-modern-primary">
                <i class="fas fa-edit mr-2"></i>Editar
            </a>
            <a href="{{ route('admin.media.index') }}" class="btn-modern-secondary">
        ...
        <div class="lg:col-span-2 space-modern">
                ...
                <div class="space-modern-sm">
                    <div class="grid-modern grid-modern-2">
                        <div>
                            ...
                            <p class="text-modern-body font-medium">{{ $media->name }}</p>
                        </div>
                        <div>
                            <label class="form-label-modern">Categoria</label>
                            <span class="badge-modern badge-modern-primary">{{ $media->category->name ?? 'Sem categoria' }}</span>
                        </div>
                    </div>
                    
                    <div>
                        <label class="form-label-modern">Descrição</label>
                        <p class="text-modern-body">{{ $media->description ?: 'Sem descrição' }}</p>
                    </div>
                    
                    <div>
                        <label class="form-label-modern">Status</label>
                        <span class="badge-modern {{ $media->status === 'active' ? 'badge-modern-success' : 'badge-modern-error' }}">
                            {{ $media->status === 'active' ? 'Ativo' : 'Inativo' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="modern-card hover-modern-lift">
                ...
                <div class="space-modern-sm">
                    ...
                    <div>
                        <h4 class="text-modern-body font-medium mb-4">Galeria de Arquivos ({{ $media->files->count() }})</h4>
        ...
        <div class="space-modern">
            ...
            <div class="modern-card hover-modern-lift">
                ...
                <div class="space-modern-sm">
                    ...
                    @php
                        $permission = $media->permissions->where('user_type_id', $userType->id)->first();
                    @endphp
                ...
                <div class="space-modern-sm">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="text-center p-4 bg-primary-50 rounded-xl">
                            <div class="text-2xl font-bold text-primary-500">{{ $media->files ? $media->files->count() : 0 }}</div>
                            <div class="text-modern-caption">Arquivos</div>
                        </div>
                        <div class="text-center p-4 bg-secondary-50 rounded-xl">
                            <div class="text-2xl font-bold text-secondary-500">{{ $media->permissions ? $media->permissions->count() : 0 }}</div>
                    ...
                    <div class="pt-4 border-t border-gray-200 space-y-3">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-modern-body">{{ $media->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-modern-caption">Atualizado em:</span>
                            <span class="text-modern-body">{{ $media->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                </div>
            </div>
